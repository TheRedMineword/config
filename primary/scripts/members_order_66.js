...
export default function (input) {
  ...
  function processMember(m) {
    ...
    const avatar = g(user, "avatar") ?? null;
    const banner = g(user, "banner") ?? null;
    const joinedAt = g(member, "joined_at") ?? null;
    const commDisabled = g(member, "communication_disabled_until") ?? null;
    const unusualDM = g(member, "unusual_dm_activity_until") ?? null;
    ...
    // Name spam
    const nameToTest = (globalName || username || "").toString();
    ...
    if (cfg.role_checks.check_for_no_roles && roles.length === 0) {
      ...
      if (cfg.server_join_checks.auto_ban_no_roles.enabled &&
          server_age_days !== null &&
          server_age_days <= cfg.server_join_checks.auto_ban_no_roles.min_server_days) {
        triggers.auto_banned_by_config = "1";
        reasons.push("Auto-ban: no roles + new account");
        risk += 35;
      }
    }
    ...
    return {
      ...
      account_age_days: account_age_days ?? null,
      server_age_days: server_age_days ?? null,
